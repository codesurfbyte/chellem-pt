...
// 영상 목록에서 첫 번째 YouTube 썸네일 URL을 반환
...
export default function VideoList({ initialData }: { initialData: ExerciseVideo[] }) {
  ...
  if (videos.length === 0) {
    return (
      <div className="card p-12 text-center text-slate text-sm">
        등록된 운동 영상이 없습니다
      </div>
    )
  }
          ...
          {filtered.map((video) => {
            ...
            return (
              <div key={video.id} className="card overflow-hidden">
                ...
                >
                  {/* 썸네일 */}
                  <div className="relative w-24 h-16 rounded-lg overflow-hidden bg-sand flex-shrink-0">
                    {thumbnail ? (
                      <Image
                        src={thumbnail}
                        alt={video.title}
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs px-2 py-0.5 rounded-full bg-brand/10 text-brand font-medium flex-shrink-0">
                        {video.category}
                      </span>
                    ...
                    <p className="text-xs text-slate/50">영상 {video.urls.length}개</p>
                  </div>

                  <span className="text-slate text-xs flex-shrink-0">{isOpen ? '▲' : '▼'}</span>
                </button>

                {isOpen && (
                  <div className="px-4 pb-4 space-y-4">
                    {video.content && (
                      <p className="text-sm text-slate whitespace-pre-wrap">{video.content}</p>
                    )}
                    {video.urls.length > 0 && (
                      <div className="space-y-3">
